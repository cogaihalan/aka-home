...
class ServerUnifiedCategoryService {
  ...
  async getCategories(params: QueryParams = {}): Promise<CategoryListResponse> {
    ...
    const response = await serverApiClient.get<CategoryListResponse>(endpoint);

    return response.data!;
  }

  async getCategory(id: number): Promise<Category> {
    const response = await serverApiClient.get<Category>(
      `${this.basePath}/${id}`
    );
    return response.data!;
  }
  ...
  async createCategory(data: CreateCategoryRequest): Promise<Category> {
    const response = await serverApiClient.post<Category>(this.basePath, data);
    return response.data!;
  }
  ...
  async updateCategory(
    ...
    const response = await serverApiClient.put<Category>(
      `${this.basePath}/${id}`,
      data
    );
    return response.data!;
  ...
  async uploadCategoryImage(
    ...
    const response = await serverApiClient.post<Category>(
