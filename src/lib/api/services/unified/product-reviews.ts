...
export class UnifiedProductReviewService {
  ...
  async getProductReviews(params: QueryParams = {}): Promise<ProductReviewListResponse> {
    ...
    const response = await apiClient.get<ProductReviewListResponse>(endpoint);
    return response.data!;
  }

  async createProductReview(data: CreateProductReviewRequest): Promise<ProductReview> {
    const response = await apiClient.post<ProductReview>(`${this.basePath}?productId=${data.productId}`, data);
    return response.data!;
  }
  ...
  async updateProductReviewStatus(
    ...
    const response = await apiClient.put<ProductReview>(`${this.basePath}/${id}?status=${data.status}`);
    return response.data!;
  }

}

export const unifiedProductReviewService = new UnifiedProductReviewService();
